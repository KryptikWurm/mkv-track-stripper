...
### Fixed
- `probe()` (both scripts) no longer treats `mkvmerge -J` exit code 1 (warnings) as a failure. mkvmerge emits valid JSON alongside warnings; previously such files were logged as un-probeable errors and skipped, even though the remux path already tolerated exit 1.
- No-default-subtitle enforcement now covers every kept subtitle track (both scripts). The remux path cleared the default flag only on the first kept sub, so a stale default flag on a later kept sub survived; and the mkvpropedit fast-path never cleared subtitle defaults at all, so an untouched file kept a default-flagged sub the remux path would have cleared. Both paths now clear the flag on all subtitle tracks.
- **mkv_strip_pp.py**: an unwritable `LOG_FILE` no longer crashes the script before processing (which exited non-zero and failed the SABnzbd job). The file handler is now best-effort with a warning; stdout logging (captured by SABnzbd) is attached first. The log file also gains `errors="surrogateescape"` to match mkvclean.
- **mkvclean**: corrupt (unparseable) checkpoint lines are now counted toward compaction, so a checkpoint containing garbage lines gets rewritten and shed at startup instead of persisting forever.
- **mkvclean**: missing `mkvmerge`/`mkvpropedit` binaries are now detected up front and abort the run, instead of logging one probe error per file across the whole batch.
