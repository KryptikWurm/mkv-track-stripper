...
- **Junk-track discrimination:** commentary, director's notes, descriptive audio for the visually impaired (audio description / narration / DVS) get stripped automatically (both scripts), whether they're tagged with the Matroska `visual impaired`, `text descriptions` or `commentary` flags or only identifiable by track name.
